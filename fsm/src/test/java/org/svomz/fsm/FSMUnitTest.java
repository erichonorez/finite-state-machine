...
import static org.svomz.fsm.FSMUnitTest.FlowBuilder.from;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.ACCEPTED;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.CLIENT_SIGNED;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.CONTRACT_OPTIONS_CHOSEN;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.CONTRACT_PRINTED;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.CONTRACT_VALIDATED;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.DIGIAL_SIGNING_CHOSEN;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.DIGITAL_SIGNED_ABANDONNED;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.DOSSIER_SCORE_INTEROGATIVE;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.DOSSIER_SCORE_NEGATIVE;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.DOSSIER_SCORE_POSITIVE;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.EMPLOYER_INFORMATION_SAVED;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.EVIDENCES_UPLOADED;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.ID_SCORE_NEGATIVE;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.ID_SCORE_POSITIVE;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.MANUAL_SIGNING_CHOSEN;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.TEMPORARY_CARD_NUMBER_ASSIGNED;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.TEMPORARY_CARD_RECEIPT_SIGNED;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowEvent.VENDOR_SIGNED;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.REFUSED;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.VALIDATED;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.WAITING_ACCEPTATION;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.WAITING_CLIENT_SIGNATURE;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.WAITING_CONTRACT_OPTIONS_CHOICE;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.WAITING_CONTRACT_PRINTING;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.WAITING_CONTRACT_VALIDATION;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.WAITING_EMPLOYER_INFORMATION;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.WAITING_EVIDENCES;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.WAITING_HOUSEHOLD_INFORMATION;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.WAITING_PERSONAL_INFORMATION;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.WAITING_SIGNATURE_TYPE_CHOICE;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.WAITING_TEMPORARY_CARD_ATTRIBUTION;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.WAITING_TEMPORARY_CARD_RECEIPT_SIGNATURE;
import static org.svomz.fsm.FSMUnitTest.MakroESignFlowState.WAITING_VENDOR_SIGNATURE;
import static org.svomz.fsm.FSMUnitTest.MyState.TO_START;
import static org.svomz.fsm.FSMUnitTest.TransitionDSL.on;
import static org.svomz.fsm.FSMUnitTest.TransitionDSL.to;
...
public class FSMUnitTest {
  ...
  public void test() {
    ...
    FSMContext context = flow.newContext();

    FSMContext toStartContext = flow.trigger(MyFlowEvent.FIXED, context);
    Assert.assertFalse(toStartContext.isFinished());
    Assert.assertEquals(toStartContext.currentState(), MyState.TO_START);

    FSMContext inProgressContext = flow.trigger(MyFlowEvent.STARTED, toStartContext);
    Assert.assertFalse(inProgressContext.isFinished());
    Assert.assertEquals(inProgressContext.currentState(), MyState.IN_PROGRESS);

    FSMContext refusedContext = flow.trigger(MyFlowEvent.REFUSED, inProgressContext);
    FSMContext wontFixContext = flow.trigger(MyFlowEvent.FIXED, refusedContext);
    Assert.assertEquals(wontFixContext.currentState(), MyState.WONT_FIX);
    Assert.assertTrue(wontFixContext.isFinished());
  }
  ...
  public void mescouilles() {
    ...
    Assert.assertEquals(MyState.TO_START, context.currentState());

    FSMContext inProgressContext = flow.trigger(MyFlowEvent.STARTED, context);
    Assert.assertEquals(MyState.IN_PROGRESS, inProgressContext.currentState());

    FSMContext toValidateContext = flow.trigger(MyFlowEvent.FIXED, inProgressContext);
    Assert.assertEquals(MyState.TO_VALIDATE, toValidateContext.currentState());

    FSMContext stillToValidateContext = flow.trigger(MyFlowEvent.NOT_VALIDATED, toValidateContext);
    Assert.assertEquals(MyState.TO_VALIDATE, stillToValidateContext.currentState());
  }
  ...
  public void mescouilles2() {
    ...
    FSMContext stillToValidateContext = flow.trigger(MyFlowEvent.NOT_VALIDATED, toValidateContext);
    Assert.assertEquals(MyState.IN_PROGRESS, stillToValidateContext.currentState());
  }
  ...
  public void itShoudGenerateAGraphViz() {
    ...
    String string = generateGraph(flow, flow.newContext());
  ...
  public void itShoudGenerateAGraphViz2() {
    ...
    FSMContext newContext = flow.trigger(MyFlowEvent.STARTED, context);

    System.out.println(generateGraph(flow, newContext));
  }
  ...
  public void MakroESignFlow() {
    ...
    System.out.println(generateGraph(makroESigningFlow, makroESigningFlow.newContext()));
    //@formatter:on
  }
  ...
  public interface FSMContext<S extends State> {
    ...
    <C extends FSMContext> C copy(S newState);
  }
  ...
  private String generateGraph(FSM flow, FSMContext context) {
  ...
  public static class FSM<C extends FSMContext> {
    ...
    public C trigger(Event event, C context) {
      ...
      return (C) context.copy(newState);
    }
    ...
    public C newContext() {
      return (C) new DefaultFSMContext(this.initialState);
    }
  ...
  public static class DefaultFSMContext implements FSMContext<State> {

    private final State currentState;

    public DefaultFSMContext(State newState) {
    ...
    @Override
    public <C extends FSMContext> C copy(State newState) {
      return (C) new DefaultFSMContext(newState);
    }
  }
  ...
  public static class MyCustomFSM extends FSM<MyCustomContext> {

    public MyCustomFSM() {
      super(
          from(MyCustomState.TODO).transit(

          on(MyCustomEvent.STARTED)
            .to(MyCustomState.WIP).transit(

            on(MyCustomEvent.FINISHED)
              .to(MyCustomState.DONE)
          )
        )
      );
    }

    public MyCustomFSM(Set<Transition> transitions, State initialState) {
      super(transitions, initialState);
    }

    public MyCustomContext newContext(String entityId) {
      return new MyCustomContext((MyCustomState) this.initialState(), entityId);
    }

  }

  private static class MyCustomContext implements FSMContext<MyCustomState> {

    private final MyCustomState currentState;
    private final String entityId;

    public MyCustomContext(MyCustomState state, String entityId) {
      this.currentState = state;
      this.entityId = entityId;
    }

    @Override
    public MyCustomState currentState() {
      return this.currentState;
    }
    ...
    @Override
    public FSMContext<MyCustomState> copy(MyCustomState newState) {
      return new MyCustomContext(newState, this.entityId);
    }
  }

  private static enum MyCustomState implements State {
    TODO(false),
    WIP(false),
    DONE(true);
  ...
  private static enum MyCustomEvent implements Event {
    STARTED,
    FINISHED;
  }

  @Test
  public void testMyCustomFSM() {

    MyCustomFSM myCustomFSM = new MyCustomFSM();
    MyCustomContext initialContext = myCustomFSM.newContext(UUID.randomUUID().toString());

    MyCustomContext wipContext = myCustomFSM.trigger(MyCustomEvent.STARTED, initialContext);
    Assert.assertEquals(MyCustomState.WIP, wipContext.currentState());

    MyCustomContext doneContext = myCustomFSM.trigger(MyCustomEvent.FINISHED, wipContext);
    Assert.assertEquals(MyCustomState.DONE, doneContext.currentState());
  }
